...
export class NSFWJS {
  ...
  public model: tf.LayersModel | tf.GraphModel | null = null
  ...
  async load() {
    ...
    const result = tf.tidy(() =>
      this.model?.predict(tf.zeros([1, size || 1, size || 1, 3]))
    ) as tf.Tensor
    await result.data()
    result.dispose()
  }
  ...
  infer(
    ...
    return tf.tidy(() => {
      ...
      const batched = resized.reshape([1, size || 0, size || 0, 3])

      let model: tf.LayersModel | tf.GraphModel | null
      if (endpoint == null) {
        model = this.model
      } else {
        if (
