  ...
  cors(req, res, async () => {
    ...
    photosSnap.forEach((photo) => {
      photos.push(
        Object.assign(photo.data(), {
          id: photo.id,
          userId: photo.ref.parent.parent.id
        })
      )
    })
    ...
    await Promise.all(
      photos.map(async (photo) => {
        ...
        if (
          requestUser &&
          requestUser.showGender !== 'both' &&
          requestUser.showGender !== user.data().gender
        ) {
          return
        }
...
exports.updatePhotoRating = functions.firestore
  .document('/users/{userId}/photos/{photoId}/votes/{documentId}')
  .onWrite(async (change, context) => {
    ...
    if (voterId) {
      ...
      if (voterSnap.exists) {
        ...
        admin
          .firestore()
          .collection(`users/${voterId}/votes`)
          .doc(photoId)
          .set(voteObj)
...
exports.onReport = functions.firestore
  .document('/users/{userId}/photos/{photoId}/reports/{documentId}')
  .onWrite(async (change, context) => {
    // Grab the current value of what was written to Firestore.
    // const snap = change.after.exists ? change.after : null;
    ...
    if (reporterId) {
      const reporterSnap = await admin
        .firestore()
        .collection('users')
        .doc(reporterId)
        .get()
...
exports.photoRemoved = functions.firestore
  ...
  .onDelete(async (snap, context) => {
    ...
    try {
      admin
        .storage()
        .bucket()
        .file(`images/${userId}/${photoObj.imageName}`)
        .delete()
      admin
        .storage()
        .bucket()
        .file(`images/${userId}/resized/${getResizedName(photoObj.imageName)}`)
        .delete()
    } catch (error) {
...
exports.photoCreated = functions.firestore
  .document('/users/{userId}/photos/{photoId}')
...
exports.scheduledFunctionExpiredPhotos = functions.pubsub
  .schedule('0 00 * * *')
...
exports.scheduledUpdatePhotos = functions.pubsub
  ...
  .onRun(() => {
    ...
    photosSnap.get().then((snapshot) => {
      ...
      snapshot.docs.forEach((doc) => {
        ...
        const happy =
          expressions.find((p) => p.expression === 'happy').probability || 0
