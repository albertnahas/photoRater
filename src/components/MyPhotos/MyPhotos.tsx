      ...
      return (
        <Grid key={p.id} item xs={view !== "two-col" ? 12 : 6} md={3}>
          <PhotoCard
            ...
            photoId={p.id}
            onClickPhoto={() =>
              photo ? setSelectedPhoto(p) : setShowForm((sf) => !sf)
            }
          />
        </Grid>
      )
          ...
          <>
            <Box sx={{ ...containerStyle, textAlign: "left" }}>
              ...
              {photosLoaded && !user?.points && (
                <Alert sx={{ my: 1 }} severity="error">
                  You can&apos;t activate your photos because you ran out of
                  tokens, please rate other photos, or buy some from your
                  profile page
                </Alert>
              )}
            </Box>
            ...
            <Grid sx={containerStyle} spacing={2} container>
              ...
              <Grid item xs={12} md={3}>
                <PhotoCard onClickShowForm={onToggleForm} view={view} />
              </Grid>
            </Grid>
          </>
        )}
